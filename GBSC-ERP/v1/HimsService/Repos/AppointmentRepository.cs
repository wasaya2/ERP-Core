...
namespace HimsService.Repos
{
      public class AppointmentRepository : RepoBase<Appointment>, IAppointmentRepository
      {
            ...
            {
                return Table.Where(a => a.AppointmentId == id).Include(a => a.Consultant).Include(a => a.VisitNature).Include(a => a.PatientInvoice).Include("PatientInvoice.PatientInvoiceItems").Include(a => a.Patient).Include("Patient.Partner").FirstOrDefault();
            }

            public List<Appointment> GetConsultantAndAppointmentdate(long id, DateTime date)
                {
